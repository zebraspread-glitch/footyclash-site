...
function getHighScoreKey(mode: StatMode) {
  return `solo_highscore_2026_${mode}`;
}
...
function getAllHighScoreEntries(): HighScoreEntry[] {
  return MODE_OPTIONS.map((option) => {
    ...
    if (typeof window !== "undefined") {
      try {
        value = Number(localStorage.getItem(getHighScoreKey(option.key)) ?? 0);
...
export default function SoloPage() {
  ...
  useEffect(() => {
    try {
      const savedMode = localStorage.getItem("solo_selected_mode_2026");
  ...
  useEffect(() => {
    try {
      localStorage.setItem("solo_selected_mode_2026", mode);
    } catch {}
  }, [mode]);

  useEffect(() => {
    try {
      const saved = Number(localStorage.getItem(getHighScoreKey(mode)) ?? 0);
  ...
  useEffect(() => {
    ...
    try {
      localStorage.setItem(getHighScoreKey(mode), String(currentScore));
    } catch {}
  }, [currentScore, highScore, mode]);
    ...
    <main className="relative min-h-screen overflow-hidden text-white">
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-3 py-5 sm:px-6 sm:py-10">
        <div className="text-center">
          ...
          <div className="mt-2 text-sm font-semibold text-white/70 sm:text-base">
            Build the best 2026 team using club spins
          </div>
        </div>
        ...
        <div className="mt-6 flex justify-center sm:mt-8">
          <div className="w-full max-w-2xl overflow-hidden rounded-[24px] border border-white/12 bg-[linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.025))] shadow-[0_20px_80px_rgba(0,0,0,0.45)] backdrop-blur-2xl sm:rounded-[28px]">
            <div className="grid grid-cols-1 md:grid-cols-2">
              <div className="px-4 py-5 sm:px-7 sm:py-7">
                ...
                <div className="mt-3 flex items-end gap-2 flex-wrap sm:mt-4">
                  <span className="break-all text-4xl font-extrabold leading-none text-white sm:text-6xl">
              ...
              <div className="border-t border-white/10 px-4 py-5 md:border-l md:border-t-0 md:border-white/10 sm:px-7 sm:py-7">
                <div className="text-[10px] font-extrabold tracking-[0.24em] text-white/45 sm:text-[11px] sm:tracking-[0.28em]">
                  HIGH SCORE
                </div>
                ...
                <button
                  ...
                  className="mt-3 flex min-h-[60px] w-full items-end justify-between gap-3 text-left sm:mt-4"
                >
                  <div className="flex min-w-0 items-end gap-2 flex-wrap">
                    <span className="break-all bg-gradient-to-b from-[#fff7c2] via-[#f2cf63] to-[#c78a18] bg-clip-text text-4xl font-extrabold leading-none text-transparent sm:text-6xl">
                      {formatStatValue(highScore, mode)}
                    </span>
                  ...
                  <span className="shrink-0 rounded-xl border border-white/15 bg-white/8 px-3 py-2 text-[11px] font-bold text-white/85 transition hover:border-white/30 hover:bg-white/12 hover:text-white sm:text-xs">
                    View All
                  </span>
                </button>
              </div>
        ...
        {gameOver && (
          <div className="mt-6 px-1 text-center sm:px-2">
            ...
            <button
              className="mt-5 min-h-[48px] w-full rounded-2xl border border-white/15 bg-white/10 px-5 py-3 text-white/90 backdrop-blur-md transition hover:border-white/30 hover:bg-white/14 sm:w-auto"
              onClick={resetGame}
            >
              Play Again
            </button>
      ...
      {showHighScores && (
        <div className="fixed inset-0 z-[60] flex items-end justify-center p-0 sm:items-center sm:p-4">
          ...
          <div className="relative flex h-[88dvh] w-full flex-col overflow-hidden rounded-t-[24px] border border-white/15 bg-zinc-950/95 p-4 shadow-[0_25px_80px_rgba(0,0,0,0.65)] backdrop-blur-xl sm:h-auto sm:max-h-[88vh] sm:max-w-2xl sm:rounded-[24px] sm:p-5">
            <div className="flex items-start justify-between gap-4 sm:items-center">
              <div className="min-w-0">
                <div className="text-lg font-extrabold tracking-[0.06em] text-white sm:text-2xl sm:tracking-[0.08em]">
                  PERSONAL HIGH SCORES
                </div>
                <div className="mt-1 text-sm text-white/55">
                  Your best score for every solo mode
                </div>
              </div>

              <button
                className="min-h-[44px] min-w-[44px] shrink-0 rounded-2xl border border-white/20 px-3 py-2 text-white/80 transition hover:border-white/40 hover:text-white"
                onClick={() => setShowHighScores(false)}
              >
                ✕
              </button>
            ...
            <div className="mt-5 min-h-0 flex-1 overflow-y-auto">
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                {allHighScores.map((entry) => (
                  <div
                    key={entry.mode}
                    className="rounded-2xl border border-white/12 bg-[linear-gradient(135deg,rgba(255,255,255,0.08),rgba(255,255,255,0.025))] px-4 py-4 shadow-[0_10px_30px_rgba(0,0,0,0.22)]"
                  >
                    ...
                    <div className="mt-3 flex items-end gap-2 flex-wrap">
                      <span className="bg-gradient-to-b from-[#fff7c2] via-[#f2cf63] to-[#c78a18] bg-clip-text text-3xl font-extrabold text-transparent drop-shadow-[0_2px_14px_rgba(242,207,99,0.18)]">
                        {formatStatValue(entry.value, entry.mode)}
                      </span>
            ...
            <div className="mt-4 flex justify-stretch sm:justify-end">
              <button
                onClick={() => refreshAllHighScores()}
                className="min-h-[48px] w-full rounded-2xl border border-white/15 bg-white/8 px-4 py-3 font-bold text-white/85 transition hover:border-white/30 hover:bg-white/12 hover:text-white sm:w-auto sm:py-2"
              >
                Refresh
              </button>
            </div>
