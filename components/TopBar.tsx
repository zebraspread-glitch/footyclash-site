"use client";

import Link from "next/link";
import Image from "next/image";
import { usePathname } from "next/navigation";

const links = [
  { href: "/", label: "Home" },
  { href: "/about", label: "About" },
  { href: "/how-to-play", label: "How to Play" },
  { href: "/contact", label: "Contact" },
  { href: "/privacy-policy", label: "Privacy" },
  { href: "/terms", label: "Terms" },
];

type Mode = "solo" | "local" | "ai" | "online" | "home";

const modeColors: Record<Mode, string> = {
  home: "bg-[#05070b]", // 👈 grey/dark for homepage
  solo: "bg-blue-600",
  local: "bg-purple-600",
  ai: "bg-orange-500",
  online: "bg-green-600",
};

export default function TopBar() {
  const pathname = usePathname();

  // 🔥 AUTO DETECT MODE FROM URL
  let mode: Mode = "home";

  if (pathname === "/") mode = "home";
  else if (pathname.startsWith("/solo")) mode = "solo";
  else if (pathname.startsWith("/local")) mode = "local";
  else if (pathname.startsWith("/ai")) mode = "ai";
  else if (pathname.startsWith("/online")) mode = "online";
  else mode = "home"; // fallback

  const bgColor = modeColors[mode];

  return (
    <header className={`${bgColor} border-b border-black/20 shadow-lg transition-colors duration-300`}>
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4 sm:px-8 lg:px-12">
        
        {/* LOGO */}
        <Link href="/" className="flex items-center gap-4 group">
          
          <div className="relative h-14 w-14 transition group-hover:scale-110">
            <Image
              src="/topbaricon.png"
              alt="FootyClash Logo"
              fill
              className="object-cover scale-125 drop-shadow-[0_0_10px_rgba(0,0,0,0.5)]"
              priority
            />
          </div>

          <div className="leading-tight">
            <div className="text-2xl font-black tracking-tight text-white">
              FootyClash
            </div>
            <div className="text-[12px] text-white/80">
            </div>
          </div>
        </Link>

        {/* NAV */}
        <nav className="flex gap-2 text-sm font-semibold">
          {links.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-full border border-white/20 bg-white/10 px-4 py-2 text-white transition 
              hover:bg-white/20 active:scale-95"
            >
              {link.label}
            </Link>
          ))}
        </nav>
      </div>
    </header>
  );
}